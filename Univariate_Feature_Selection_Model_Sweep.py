...
#tax-credit-code (git clone https://github.com/caporaso-lab/tax-credit-code.git)
#tax-credit-data (git clone https://github.com/caporaso-lab/tax-credit-data.git)
...
project_dir = join('tax-credit-data')
...
def build_pipeline(classifier, hash_params, feat_sel_params, classify_params):
    return Pipeline([
        ...
        ('feat_sel', SelectKBest(feat_sel_params)),
        ('classify', classifier(**classify_params))])
    
# Now fit the pipelines.
pipelines = {'q2-NB-Univariate': build_pipeline(
...
#dataset_reference_combinations = [
 #('mock-8', 'gg_13_8_otus'), # formerly Broad-1
#]
#method_parameters_combinations = {
 #   'q2-NB-Univariate': {'confidence': [0.7],
  #                       'classify__alpha': [0.01],
...
taxonomy_glob = join(results_dir, 'mock-*', 'gg_13_8_otus', 'q2-NB-Univariate', '*', 'taxonomy.tsv')
